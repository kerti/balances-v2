...
- A pinned release tag. The image is published to the GitHub Container Registry as
  `ghcr.io/kerti/balances:<tag>` (e.g. `v0.9.0-alpha.5`). **No `latest` tag is published** — you always pin a
...
```sh
BALANCES_TAG=v0.9.0-alpha.5  # the release you want to run
...
|---|---|---|
| `BALANCES_TAG` | `v0.9.0-alpha.5` | Released image tag to run. Pin a real version; bump to upgrade. |

...
1. **M7 = productization → `v0.7.0-alpha.1`** (minor bump = milestone boundary, ADR-0033). Make it
   ...
   and sliced into #224–229**: #224 (`APP_URL` single-origin collapse) **DONE**; next #225 (GHCR
   publish), #226 (`EMAIL_ENABLED` + copy-invite-link, also #223's first flag) — no blockers — then

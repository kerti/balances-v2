...
var receivableImportHeader = []string{"year_month", "as_of_date", "amount", "currency", "description"}

func TestReceivableHandlers_ImportTemplate(t *testing.T) {
	h := newHarness(t)
	parent := h.createReceivable(t, "Template target")
	base := "/receivables/" + parent.ID.String() + "/snapshots"
	...
	t.Run("404 unknown receivable", func(t *testing.T) {
		rec := h.do(t, "GET", "/receivables/"+uuid.NewString()+"/snapshots/import-template", nil)
		requireStatus(t, rec, http.StatusNotFound)
	})

	t.Run("400 invalid id", func(t *testing.T) {
		rec := h.do(t, "GET", "/receivables/not-a-uuid/snapshots/import-template", nil)
		requireStatus(t, rec, http.StatusBadRequest)
	})
}

func TestReceivableHandlers_ImportSnapshots(t *testing.T) {
	h := newHarness(t)
	parent := h.createReceivable(t, "Import target")
	base := "/receivables/" + parent.ID.String() + "/snapshots"

	validRows := func() [][]string {
		return [][]string{
			receivableImportHeader,
			{"2026-01", "2026-01-31", "3000000", "IDR", "Jan"},
			{"2026-02", "2026-02-28", "2500000", "", "Feb"}, // blank currency -> native
		}
	}
	...
	t.Run("commit with bad row is 422 and writes nothing", func(t *testing.T) {
		other := h.createReceivable(t, "Bad-row target")
		obase := "/receivables/" + other.ID.String() + "/snapshots"
		rows := [][]string{
			receivableImportHeader,
			{"not-a-month", "2026-01-31", "3000000", "IDR", "bad"},
		}
	...
	t.Run("404 unknown receivable", func(t *testing.T) {
		path := "/receivables/" + uuid.NewString() + "/snapshots/import"
		rec := h.doUpload(t, path, buildImportXLSX(t, validRows()))
		requireStatus(t, rec, http.StatusNotFound)
	})

	t.Run("400 invalid id", func(t *testing.T) {
		rec := h.doUpload(t, "/receivables/not-a-uuid/snapshots/import", buildImportXLSX(t, validRows()))
		requireStatus(t, rec, http.StatusBadRequest)
	})

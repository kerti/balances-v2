...
// covers: INV-SNAPSHOTS-06
func TestLiabilitySnapshotHandlers_BulkValidation(t *testing.T) {
	h := newHarness(t)
	l := h.createLiability(t, "Validation parent", "personal")
	goodRow := []map[string]any{{"liability_id": l.ID.String(), "amount": "1000", "currency": "IDR"}}
	...
	cases := []struct {
		...
		{"row missing currency", map[string]any{"year_month": "2026-05", "rows": []map[string]any{{"liability_id": l.ID.String(), "amount": "1000"}}}, http.StatusBadRequest},
		{"row bad uuid shape", map[string]any{"year_month": "2026-05", "rows": []map[string]any{{"liability_id": "not-a-uuid", "amount": "1000", "currency": "IDR"}}}, http.StatusBadRequest},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			requireStatus(t, h.do(t, "POST", "/liabilities/snapshots/bulk", tc.body), tc.want)
		})
	}

	t.Run("entry list rejects a bad year_month", func(t *testing.T) {
		requireStatus(t, h.do(t, "GET", "/liabilities/snapshots/entry?year_month=nope", nil), http.StatusBadRequest)
	})
	t.Run("entry list rejects a future year_month", func(t *testing.T) {
		requireStatus(t, h.do(t, "GET", "/liabilities/snapshots/entry?year_month=2030-02", nil), http.StatusBadRequest)
	})
}
...
// covers: INV-SNAPSHOTS-07
func TestLiabilitySnapshotHandlers_EntryList_SoleOwner(t *testing.T) {
	h := newHarness(t)
	rec := h.do(t, "POST", "/liabilities", map[string]any{
		"display_name":       "Alice's private loan",
		"subtype":            "personal",
		"ownership_type":     "sole",
	...
	got := h.do(t, "GET", "/liabilities/snapshots/entry?year_month=2026-05", nil)
	...
	if len(body.Rows) != 1 {
		t.Fatalf("want 1 entry row for the sole liability, got %d", len(body.Rows))
	}
	if body.Rows[0].SoleOwnerUserID == nil || *body.Rows[0].SoleOwnerUserID != h.user.ID.String() {
		t.Errorf("sole-owned liability entry row: want sole_owner_user_id %s, got %v", h.user.ID, body.Rows[0].SoleOwnerUserID)
	}
}

...
var liabilityImportHeader = []string{"year_month", "as_of_date", "amount", "currency", "description"}

func TestLiabilityHandlers_ImportTemplate(t *testing.T) {
	h := newHarness(t)
	parent := h.createLiability(t, "Template target", "personal")
	base := "/liabilities/" + parent.ID.String() + "/snapshots"
	...
	t.Run("404 unknown liability", func(t *testing.T) {
		rec := h.do(t, "GET", "/liabilities/"+uuid.NewString()+"/snapshots/import-template", nil)
		requireStatus(t, rec, http.StatusNotFound)
	})

	t.Run("400 invalid id", func(t *testing.T) {
		rec := h.do(t, "GET", "/liabilities/not-a-uuid/snapshots/import-template", nil)
		requireStatus(t, rec, http.StatusBadRequest)
	})
}

func TestLiabilityHandlers_ImportSnapshots(t *testing.T) {
	h := newHarness(t)
	parent := h.createLiability(t, "Import target", "personal")
	base := "/liabilities/" + parent.ID.String() + "/snapshots"
	...
	validRows := func() [][]string {
		return [][]string{
			...
			{"2026-02", "2026-02-28", "4800000", "", "Feb"}, // blank currency -> native
		}
	}
	...
	t.Run("commit writes rows", func(t *testing.T) {
		...
		// Re-committing the same months classifies as updates, not inserts.
	...
	t.Run("commit with bad row is 422 and writes nothing", func(t *testing.T) {
		other := h.createLiability(t, "Bad-row target", "personal")
		obase := "/liabilities/" + other.ID.String() + "/snapshots"
		rows := [][]string{
			liabilityImportHeader,
			{"not-a-month", "2026-01-31", "5000000", "IDR", "bad"},
		}
	...
	t.Run("preview surfaces row errors without 422", func(t *testing.T) {
		rows := [][]string{
			liabilityImportHeader,
			{"nope", "2026-01-31", "5000000", "IDR", "bad"},
		}
		rec := h.doUpload(t, base+"/import", buildImportXLSX(t, rows))
		requireStatus(t, rec, http.StatusOK)
		if body := decodeBody[importResp](t, rec); len(body.Errors) == 0 {
			t.Error("expected row errors surfaced in preview")
		}
	})
	...
	t.Run("404 unknown liability", func(t *testing.T) {
		path := "/liabilities/" + uuid.NewString() + "/snapshots/import"
		rec := h.doUpload(t, path, buildImportXLSX(t, validRows()))
		requireStatus(t, rec, http.StatusNotFound)
	})

	t.Run("400 invalid id", func(t *testing.T) {
		rec := h.doUpload(t, "/liabilities/not-a-uuid/snapshots/import", buildImportXLSX(t, validRows()))
		requireStatus(t, rec, http.StatusBadRequest)
	})

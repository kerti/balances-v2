...
func TestInvestmentHandlers_ImportTemplate(t *testing.T) {
	...
	base := "/investments/" + stock.Investment.ID.String() + "/snapshots"
	...
	t.Run("404 unknown investment", func(t *testing.T) {
		rec := h.do(t, "GET", "/investments/"+uuid.NewString()+"/snapshots/import-template", nil)
		requireStatus(t, rec, http.StatusNotFound)
	})

	t.Run("400 invalid id", func(t *testing.T) {
		rec := h.do(t, "GET", "/investments/not-a-uuid/snapshots/import-template", nil)
...
func TestInvestmentHandlers_ImportSnapshots(t *testing.T) {
	...
	// quantity × price_per_unit derives amount; currency blank -> native (IDR).
	...
	t.Run("commit with bad row is 422 and writes nothing", func(t *testing.T) {
		other := h.createStock(t, "Bad-row stock")
		obase := "/investments/" + other.Investment.ID.String() + "/snapshots"
		rows := [][]string{
			qtyPriceHeader,
			{"not-a-month", "2026-01-31", "100", "9500", "IDR", "bad"},
		}
	...
	t.Run("404 unknown investment", func(t *testing.T) {
		path := "/investments/" + uuid.NewString() + "/snapshots/import"
		rec := h.doUpload(t, path, buildImportXLSX(t, validRows()))
		requireStatus(t, rec, http.StatusNotFound)
	})

	t.Run("400 invalid id", func(t *testing.T) {
		rec := h.doUpload(t, "/investments/not-a-uuid/snapshots/import", buildImportXLSX(t, validRows()))
		requireStatus(t, rec, http.StatusBadRequest)
	})

...
test("link an existing time deposit as the rollover successor", async ({ page }) => {
  ...
  await expect(page.getByRole("row", { name: new RegExp(successorName) })).toBeVisible();
  ...
  await expect(linkDialog.getByText("Link the rollover deposit")).toBeVisible();
  await linkDialog.getByTestId("rollover-successor-select").selectOption({ label: successorName });
  ...
  await page
    ...
    .click();
  await expect(page.getByRole("heading", { level: 1, name: successorName })).toBeVisible();
  await page.getByRole("button", { name: "Delete" }).click();
  await page.getByRole("alertdialog").getByRole("button", { name: "Delete" }).click();

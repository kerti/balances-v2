...
export function SignInScreen() {
  ...
  // Public-demo notice (ADR-0041, #346/CF-03): the demo is a shared, public,
  // writable sandbox behind one login, so state the terms up front — shared
  // account, no real data, nightly wipe, no warranty, and the maintainer-run
  // out-of-cycle reset (requested via the project link in the footer).
  // `restore-commit` stays live on demo (accept + document, ADR-0041) — this
  // notice is the documented acceptance of the defacement-until-reset window.
  //
  // Placement is by breakpoint (ADR-0050 amendment): on desktop it rides
  // AuthLayout's `aside` slot for room to breathe beside the hero; on phones it
  // sits in the Card's flow where it belongs. `useIsMobile` mounts exactly one
  // of the two, so the `signin-demo-notice` testid is never duplicated.
  const demoNotice = methods?.demo_mode ? (
    <div
      data-testid="signin-demo-notice"
      role="note"
      className="space-y-2 rounded-lg border border-border bg-muted/50 p-3 text-xs text-muted-foreground md:bg-background/60 md:p-4 md:text-sm"
    >
  ...
  return (
    <AuthLayout aside={isMobile ? undefined : demoNotice}>
      <Card className="w-full max-w-md">
        <CardHeader>
          <AppLogo className="w-full h-auto md:hidden" />
          {/* Redundant with the desktop hero tagline, so mobile-only. */}
          <CardDescription className="md:hidden">{t("signIn.tagline")}</CardDescription>
        </CardHeader>

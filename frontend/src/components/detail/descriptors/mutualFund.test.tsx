// Per-type conformance for the MutualFund detail descriptor (ADR-0051, A4 —
// qty×price completion). A mechanical repeat of the A3 Stock conformance: it
// drives the generic `PositionDetailScreen` with the *real* descriptor over
// MSW-stubbed endpoints and asserts every region the hand-written page rendered
// still renders — the web-parity net for the consolidation.
...
const investment: Investment = {
  ...
  description: "Core mutual fund holding",
  subtype: "mutual_fund",
  ownership_type: "sole",
...
const details: MutualFundDetails = {
  investment_id: "i1",
  fund_code: "GEF001",
  fund_manager: "Acme Asset Mgmt",
  fund_type: "equity",
};
...
function stubEndpoints() {
  server.use(
    http.get("/api/investments/mutual-funds/i1", () => HttpResponse.json({ investment, details })),
...
describe("mutualFundDescriptor detail (conformance)", () => {
  it("renders every region through the generic shell", async () => {
    ...
    expect(within(txnCard).getByText("Annual distribution")).toBeInTheDocument();
    ...
    await userEvent.type(search, "distribution");
    expect(within(txnCard).getByText("Annual distribution")).toBeInTheDocument();
    expect(within(txnCard).queryByText("Opening purchase")).not.toBeInTheDocument();

    // Actions row + the export anchor (mutual-fund-prefixed).
    ...
    expect(screen.getByTestId("mutual-fund-export")).toHaveAttribute(
      "href",

...
// ErasePanel is the founder's whole-household hard delete (ADR-0040/#300) — the
...
// RestorePanel's uploaded-file summary), so this is a single request, not a
// preview/commit pair — the household's real name is already on screen before
// the user types it.
//
// It is the one irreversible action on this screen, so it takes the panel
// group's destructive tone: a `bg-destructive/10` wash across the whole band
// rather than the thin outline it used to wear as a card, which read as level
// with "Download backup" once the three were stacked into one card.
//
...
export function ErasePanel() {
  ...
  return (
    <SettingsPanel
      tone="destructive"
      title={t("data.erase.title")}
      description={t("data.erase.description")}
    >
      {!open ? (
        <PanelActions>
          <Button
        ...
        <div className="space-y-4" data-testid="erase-confirm">
          <div
            className="rounded-md border border-destructive/50 bg-card p-3 text-sm"

...
import { EditTradeTransactionDialog } from "@/components/dialogs/EditTradeTransactionDialog";
import { EditCashIncomeTransactionDialog } from "@/components/dialogs/EditCashIncomeTransactionDialog";
import { EditFeeTransactionDialog } from "@/components/dialogs/EditFeeTransactionDialog";
import { EditMaturityTransactionDialog } from "@/components/dialogs/EditMaturityTransactionDialog";
...
// Sign convention for the Cash impact figure:
...
// The shared investment-transaction row logic (ADR-0051 Phase B): edit/delete
// dialog state, the formatted date / type / detail / cash-impact text, and the
// shape-conditional dialog nodes, single-sourced so the desktop
// `TransactionRow` and the mobile `TransactionCard` present the same values,
// open the same dialogs, and can never drift between renderers — the
// `useAccruedInterestSnapshotRow` idiom applied to the buy/sell/coupon ledger
// shape shared by all five investment types.
export function useTransactionRow<TUpdate, TDelete>(
  ...
  return {
    dateText: formatDate(transaction.transaction_date),
    label,
    detail: detailLine(),
    impactText: impactText(),
    impactColorClass: impactColor(dir),

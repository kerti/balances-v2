...
## Amendment — 2026-08-06: bank interest fails this ADR's own realized-cash test

The passive-cash scope above (§"Passive income has two named scopes") admits `Interest` on the grounds
that it is "an external cash stream" and "cash that has *left* any instrument". **That reasoning is
wrong.** Bank interest is *credited into the very account that earned it*: checked against real data, in
a month where nothing else moved the account, the entire month-over-month balance change **was** the
credited interest, to the rupiah. The interest was never paid out to a separate account — a deposit
ladder's interest likewise stayed inside the deposit, while the linked checking account showed only
its own negligible drip. Rent and pension arrive from outside the household's balance sheet; bank
interest is
generated by a balance already counted in `nw_assets` and stays there. It is an accrual, not realized
external cash — the same distinction this ADR already draws between paid-out and accruing bond coupons
(Amendment 2026-07-20), applied consistently.

It is also **negligible**. Real-world current/savings rates make the term a rounding error: on 100M of
bank balances at ~0.5%/yr against 10M/month of expenses it moves Cash-Flow by +0.04pp, Passive-Income by
+0.42pp, and Fund Resilience by +0.4% — always optimistic, invisible after rounding. Where the term is
*not* negligible, that is the signal that an interest-bearing instrument is untracked (a time deposit
modelled as a `bank_account` because its placement/maturity/rate were lost), i.e. a data gap rather than
a passive income stream.

### Decision

`interest` is **income the household does not rely on**. The category-level default flips to
`incidental` (`categoryDefaultRegularity`), which keeps it out of `earned_income_total_routine` and
therefore out of every statistics income term, per INV-FINANCE-24. The category is **retained** and
still counts toward net worth, the income statement and LivingExpenses: `bank_account` assets feed no
`asset_value_change` (only `property`/`vehicle` do), so without an income row the credited interest
reads as an unfunded net-worth rise and pushes derived living expenses negative. Bookkeeping
counterpart, not a ratio input.

The **passive-cash formula is deliberately left unchanged** — `Interest` stays in the
`Rental + Pension + Interest` sum in `buildStats`. Removing it needs a migration-free but
invariant-touching change whose payoff is the sub-1pp figures above, and one population would lose a
genuinely external stream: households lending on P2P or to a person have **no receivable-return term**
in the engine (receivables reach `monthly_reports` only through `nw_receivables`), so an `interest`
income row is the only home their lending yield has. Fixing *that* is the real gap and is tracked as
**#608**; until then, leaving the formula alone keeps those users whole at a negligible cost to
everyone else. Revisit if a household turns up with material, genuinely-paid-out interest.

For a household that tracks its instruments properly, deposit yield never reaches this category at all:
it flows through the time-deposit instrument as investment return, into `g`. Users who take the
`bank_account` shortcut instead have their yield land in earned income and LivingExpenses, so their
ratios **understate** — the safe direction, and an accepted cost of the shortcut rather than a defect.

### Invariants

No new invariant, and INV-FINANCE-19/21/24 are unchanged: this moves a category's *default* regularity,
which those invariants already cover generically. No migration, no `engine_version` bump — no engine
input or output changes, and existing rows keep whatever regularity the user chose.

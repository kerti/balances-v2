...
| [0042](0042-project-license-agpl-3-0.md) | Project license: AGPL-3.0; self-hosting stays unrestricted, network copyleft blocks closed-source competing hosted forks, explicit patent grant; no SPDX retrofit, no CLA until first external contributor | license, legal |
| [0043](0043-descriptor-driven-position-list-screens.md) | Ten per-type list screens + eleven `*ListRow`s collapse into one descriptor-driven `PositionListScreen` core + two cluster presets over the Position **shared surface** (group-specific bits injected as slots, never absorbed — not in tension with 0022's storage non-polymorphism); presentation-neutral descriptor feeds dense web table + compact mobile cards; form fields stay hand-written; activates 0021's RTL+MSW component tier (#69) with a synthetic-descriptor core test; parallel-run per-route rollout | frontend, ui, tests |
| [0044](0044-client-side-pdf-export-of-monthly-reports.md) | PDF export of monthly reports renders client-side via `@react-pdf/renderer`, mirroring the dashboard's aggregate content exactly (no per-position ledger, no pagination logic); chart redrawn natively as vector, not rasterized; emailing composes on top later via an attachment endpoint, fully-unattended scheduled send explicitly deferred — scope amended by 0045 | reports, pdf, frontend |
| [0045](0045-detailed-pdf-report-layout.md) | Supersedes 0044 on render location: the PDF report moves server-side, rendered natively in Go (`go-pdf/fpdf`) as a portrait, itemized financial statement — per-Position breakdown, hand-drawn vector composition donuts + trend, all currencies surfaced at position level, Geist embedded, server-side `x/text` money formatting guarded by a golden parity test vs `Intl`; `GET /api/reports/{yearMonth}/pdf` (locale from user preference, no currency param) replaces the client renderer and the `/positions` JSON endpoint; drops the ~1.4MB react-pdf chunk (#394); ratios/statistics deferred (#412) | reports, pdf, backend |
| [0046](0046-bulk-monthly-entry-mode.md) | Recurring monthly snapshot ritual becomes an **entry mode on the 0043 descriptor lists** (per-type, shape from each type's descriptor — dissolves Investment's two-shape screen), not a new screen; statement-aligned per-type batching; **Save persists only dirty rows**, untouched positions ride the carry-forward rule (no per-month stamp), re-entry upserts the `(position, year_month)` unique row; one batch-level `year_month` + `as_of_date` (seeded from `carryover_date_mode`), currency inherited/read-only; month-aware eligibility (active + this-month-terminated, window-bounded); atomic per-type batch write + one coalesced report regen (0006); sliced amount-only→qty×price→accrued; no migration | frontend, ui, snapshots, reports |

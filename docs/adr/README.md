...
| [0045](0045-detailed-pdf-report-layout.md) | Amends 0044: PDF report gains an itemized per-position breakdown, composition charts, and a 12-month expense-vs-passive-income trend; new `GET /api/reports/{yearMonth}/positions` endpoint extracts the report engine's already-computed per-position values instead of re-deriving them client-side; pagination becomes real document flow; ratios/statistics panel deferred (#412), current-vs-non-current split out of reach without new backend classification | reports, pdf, frontend, backend |

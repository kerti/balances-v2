...
| INV-STALENESS-04 | Engine-version regen — every materialized row carries the `engine_version` stamp that wrote it, and `needsRegen` forces regeneration when a row's stamp differs from the current `reportEngineVersion` (NULL — a row from before the stamp existed, e.g. pre-migration-00012 — counts as different). This closes the watermark's blind spot: an engine-logic or report-schema change touches no input row, so the data-driven watermark alone would serve the old rows forever. A matching stamp with an unmoved watermark stays a cache hit | ADR-0006, ADR-0048 | Critical |

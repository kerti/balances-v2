...
| INV-JOURNEYS-05 | Asset bulk monthly-entry saves and reflects in the browser — from Assets home, "Enter this month" lists an eligible position with its carried-forward prefill and a live changed-count; typing a value marks the row dirty and ticks the count, and Save all commits the dirty rows and returns to Assets home. Re-opening the same month proves the batch persisted: the row now carries this month's own value and warns the next save overwrites it. The browser/journey face of INV-SNAPSHOTS-06 (dirty-only atomic save) and INV-SNAPSHOTS-07 (carry-forward surfacing); those own the server contract and its unit-level face, this owns the list→edit→save→re-open loop the non-technical monthly-update user actually walks. Verified in `bulk-monthly-entry.spec.ts` | ADR-0046 / INV-SNAPSHOTS-06·07 | High | `@smoke` |

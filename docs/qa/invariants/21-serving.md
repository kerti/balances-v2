...
| INV-SERVING-07 | `/healthz` never echoes the raw DB driver error to the client: a DB failure logs the error server-side and returns a bare `503` with no error text in the body, per the same no-message-on-the-wire convention ADR-0027 uses elsewhere | #364 | Medium |

...
| INV-SNAPSHOTS-07 | The bulk monthly-entry list surfaces every eligible position with its carry-forward prefill — the most-recent snapshot at or before the target month (null when the position has no history yet), the position's native currency, and the month it was carried from — excluding positions terminated before the target month | ADR-0046 | Medium |

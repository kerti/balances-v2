...
| SERVING | Single-origin static-serving seam — the Go binary serving the built SPA from `WEB_DIR` beside `/api` (ADR-0030). Deep-link/refresh resolves to the SPA shell (ADR-0025), a missing content-hashed chunk 404s rather than mis-serving `200 text/html` (#190), extensionless client routes under `/assets/` still fall back (#241), and the `/*` catch-all never shadows `/api`/`/healthz`. Guards the hosting seam past PRESENTATION, not the rendered SPA (#244) | High/Medium | [catalog](invariants/21-serving.md) | [coverage](coverage/21-serving.md) |

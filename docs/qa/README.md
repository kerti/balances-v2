...
| NOTIFICATIONS | _Seeded next — transactional email delivery (`internal/email`, ADR-0020: Mailer behind an interface, SMTP/Mailpit dev → Resend prod). Sits **beneath AUTH's invite-token invariants** (INV-AUTH-06/07/08 own the token; this owns the message that carries it). Candidate rows: (1) **invite mailed to the right address only** — `sendInvitationEmail` sends `To: invite.InvitedEmail` with an accept URL embedding the issued token; a misaddressed message would leak a working invite token to the wrong person (privacy/security, meets the bar); (2) **best-effort, non-blocking** — a `mailer.Send` failure is logged and the invitation still persists + the 201 still returns the AcceptURL (email failure must never lose the invite or block creation — the deliberate ADR-0020 "best-effort" contract); (3) **HTML-escaped interpolation** — inviter/household display names are `htmlEscape`d into the HTML body (injection guard). Likely **new work**: a fake Mailer capturing the Message to assert To/token/best-effort; `smtp_test.go` only covers the SMTP impl, not the invitation call site. Targets: `internal/auth/invitations.go`, a new `invitations_email_test.go`. Source: ADR-0020 (mailer interface), ADR-0017 (invitations)_ | — | [catalog](invariants/18-notifications.md) | — |

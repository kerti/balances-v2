...
| SOFT-DELETE | _Seeded — the cross-cutting `deleted_at IS NULL` discipline: soft+idempotent delete, deleted rows invisible to every read path (List/Export/history gaps), report gather excludes tombstones, delete-then-recreate-same-month clean (ADR-0005/0006)_ | — | [catalog](invariants/12-soft-delete.md) | — |

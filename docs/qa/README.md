...
| STALENESS | _Seeded — cache-coherence over materialized `monthly_reports`: watermark-driven regen never serves a report computed from since-changed inputs, month-range pruning, whole-household atomic recompute. Distinct from FINANCE (the engine number) and its carry-forward/stale-flag (ADR-0006)_ | — | [catalog](invariants/13-staleness.md) | — |

...
| JOURNEYS | _Seeded next — the **E2E-native** companion to PRESENTATION: whole-browser user journeys a handler unit test or a pure-fn vitest can't reach, where the invariant lives in the *round-trip* across redirect/session/navigation boundaries. The charter row is the OAuth sign-in flow (ADR-0024): button→provider redirect→callback→session cookie→landed-and-authenticated, of which only the callback half is unit-tested today (see AUTH); the full loop is exercised against the mock-OIDC server. Other candidates: session expiry→re-auth, the import preview→commit→list round-trip through the UI (the browser face of IMPORT's preview/commit parity), and the carryover dialog seeding a submittable form (the journey face of INV-PRESENTATION-02). **Gate wrinkle (`how-it-works.md`): Playwright is tiered — only `@smoke`-tagged specs gate per-PR, the full suite runs nightly (#70).** So an invariant covered only by a non-smoke spec would be credited by a future per-PR `-strict` gate without running in that PR. Before seeding annotations here, either tag the covering spec `@smoke` so it runs in the gate, or pin which rows are nightly-verified-by-design. Target specs: `frontend/e2e/*.spec.ts`. Source: ADR-0024 (OAuth), #70 (tiered E2E in CI)_ | — | [catalog](invariants/17-journeys.md) | — |

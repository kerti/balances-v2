...
| ATTRIBUTION | _Seeded — per-owner routing beneath FINANCE's `userBreakdowns`: `ownerKey` sends sole→owner bucket, joint→whole `jointKey` bucket (never split), membership seeds the bucket set, malformed-sole degrades to joint. Intra-household misallocation that still reconciles — distinct from FINANCE-02 (the total) and TENANCY (cross-household leak) (ADR-0004/0012/0005)_ | — | [catalog](invariants/14-attribution.md) | — |

...
| INV-LIFECYCLE-03 | An investment terminal flip writes a truthful 0-value close snapshot at the termination month (the INV-FINANCE-11/-13 read-side assumption) | `backend/internal/repo/investment_import_create_test.go`<br>`backend/internal/repo/investment_maturity_edit_test.go`<br>`backend/internal/repo/lifecycle_close_snapshot_test.go`<br>`frontend/e2e/lifecycle-snapshot.spec.ts` |
